    ...
    <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
      ...
      {recentJobs.length === 0 ? (
        <p className="text-xs leading-relaxed text-slate-500">
          Jobs you open will appear here for quick access.
        </p>
      ) : (
        <div className="space-y-3">
          {recentJobs.slice(0, 3).map((job) => (
            <Link
              ...
              className="p-3 bg-[#f8f9ff] hover:bg-[#eff4ff] rounded-xl border border-slate-200/50 transition-colors flex items-center justify-between group"
            >
